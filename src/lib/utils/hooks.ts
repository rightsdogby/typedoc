...
export class EventHooks<T extends Record<keyof T, unknown[]>, R> {
    ...
    /**
     * Emits an event to all currently subscribed listeners, passing the result of the first
     * listener to the next listener.
     * @param event the event to emit.
     * @param args any arguments required for the event.
     */
    reduce<K extends KeysOfType<T, [R]>>(event: K, initial: R): R {
        const listeners = this._listeners.get(event)?.slice() || [];
        this._listeners.set(
            event,
            listeners.filter(({ once }) => !once)
        );
        return listeners.reduce((acc, { listener }) => listener(acc), initial);
    }
